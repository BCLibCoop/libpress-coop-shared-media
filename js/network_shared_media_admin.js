 ...
 ;(function($,window) {
	...
	CoopNSM.prototype = {
		...
		add_nsm_button: function() {
			...
			$('#coop_nsm_btn').click( self.mask_overlay );
		},
			...
			$.post( ajaxurl, data ).complete(function(r){			
				...
				if( res.images !== null ) {
					...
					for( var i=0;i<imgs.length;i++) {
						...
						var d = $('<div class="coop-nsm-image-frame" data-img-id="' + imgs[i].id + '"/>').append(img).click( self.toggle_image_selection );
						imgbox.append( d );
					}
				}
			});
		...
		fetch_image_metadata: function( div ) {
			...
			// this is not an editable context for the users 
			//	var href = '/wp-admin/post.php?post=' + img_id + '&action=edit&image-editor';
			//	$('.coop-nsm-item-meta-imglink').attr('href',href);
							
				$('.coop-nsm-item-meta-imgname').empty().append(self.fs.file);
				$('.coop-nsm-item-meta-imgdate').empty().append(self.date);
				$('.coop-nsm-item-meta-imgsize').empty().append(self.fs.width + ' x ' + self.fs.height);
		...
		**/
		insert_layout: function() {
			...
			var hdr  = $('<div class="coop-nsm-header"/>');
				...
				hdr.append($('<a href="#" id="coop_nsm_close">Close</a>').click( self.remove_overlay ));
		...
		//	var imgdele = $('<a class="coop-nsm-item-meta-imgdele" href="">Delete Permanently</a>');
			...
			var btn = $('<p>&nbsp;</p><button id="coop-nsm-img-insert-btn" class="button button-primary button-large">Insert Image</button>');
				btn.click( self.insert_image );
		...
		**/
		insert_image: function() {
			...
			var img = anchor + '<img class="' + alignment + '" src="' + self.server + '/wp-uploads/' + self.dir + imgfile + '" width="'+w+'" height="' + h + '" >' + tail;
